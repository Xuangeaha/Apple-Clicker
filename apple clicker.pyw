...
def achievement_logic():
    ...
    while whao == 0:
        ...
        if achievement_36_on == 1: achievement_36_get = 1; achievement_get("你的后院", "购买 1 个 果园")
        if achievement_37_on == 1: achievement_37_get = 1; achievement_get("开垦荒地", "购买 5 个 果园")
        if achievement_38_on == 1: achievement_38_get = 1; achievement_get("", "购买 10 个 果园")
        if achievement_39_on == 1: achievement_39_get = 1; achievement_get("", "购买 50 个 果园")
        if achievement_40_on == 1: achievement_40_get = 1; achievement_get("终极奉献", "购买 100 个 果园") 
        
        if achievement_41_on == 1: achievement_41_get = 1; achievement_get("", "购买 1 个 水果镇")
        if achievement_42_on == 1: achievement_42_get = 1; achievement_get("", "购买 5 个 水果镇")
        if achievement_43_on == 1: achievement_43_get = 1; achievement_get("", "购买 10 个 水果镇")
        if achievement_44_on == 1: achievement_44_get = 1; achievement_get("", "购买 50 个 水果镇")
        if achievement_45_on == 1: achievement_45_get = 1; achievement_get("", "购买 100 个 水果镇")

        if achievement_46_on == 1: achievement_46_get = 1; achievement_get("工业时代", "购买 1 个 苹果工厂")
        if achievement_47_on == 1: achievement_47_get = 1; achievement_get("半自动苹果机", "购买 5 个 苹果工厂")
        if achievement_48_on == 1: achievement_48_get = 1; achievement_get("全自动苹果机", "购买 10 个 苹果工厂")
        if achievement_49_on == 1: achievement_49_get = 1; achievement_get("工业革命", "购买 50 个 苹果工厂")
...
button_glass = tk.Button(frame_button, text="购买 反物质聚光镜", cursor='hand2', overrelief='sunken', command=glass_button_clicked)
...
bottom_button_download = tk.Button(frame_bottom_left, text="导出存档..", width=10, height=1, pady=3,
                                   cursor='hand2', overrelief='sunken', command=window_download)  # 底部按钮
bottom_button_download.grid(row=1, column=2, sticky='w')

blank_a = tk.Label(frame_bottom_left, text=" ")
blank_a.grid(row=1, column=3)

bottom_button_upload = tk.Button(frame_bottom_left, text="导入存档..", width=10, height=1, pady=3,
...
bottom_button_about = tk.Button(frame_bottom_left, text="退出..", width=8, height=1, pady=3,
...
def window_settings():
    ...
    radio = tk.Radiobutton(set_frame_1,variable=r, value='1', text='Radio1')
    radio.grid(row=1,column=2)

    radio = tk.Radiobutton(set_frame_1, variable=r, value='2', text='Radio2')
